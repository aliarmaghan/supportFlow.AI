            ...
            if conversation:
                ...
                if metadata and 'classification' in metadata:
                    ...
                    if isinstance(classification, dict):
                        # #region agent log
                        _log_debug(
                            "production_memory.py:122",
                            "Before updating escalation flag",
                            {
                                "conversation_id": conversation_id,
                                "current_escalated": conversation.escalated,
                                "current_status": conversation.status,
                                "new_requires_escalation": classification.get('requires_human_escalation', False)
                            },
                            "A"
                        )
                        # #endregion
                        ...
                        # BUG FIX: Once escalated, don't overwrite to False
                        # Only set to True if required, preserve existing True value
                        if classification.get('requires_human_escalation', False):
                            conversation.escalated = True
                        # If already escalated, keep it escalated (don't set to False)
                        # #region agent log
                        _log_debug(
                            "production_memory.py:135",
                            "After updating escalation flag",
                            {
                                "conversation_id": conversation_id,
                                "final_escalated": conversation.escalated,
                ...
                if metadata and 'articles_used' in metadata:
                    # #region agent log
                    _log_debug(
                        "production_memory.py:125",
                        "Tracking KB articles",
                        {
                            "conversation_id": conversation_id,
                            "articles_count": len(metadata['articles_used']),
                            "article_ids": [a.get('id') for a in metadata['articles_used']]
                        },
                        "B"
                    )
                    # #endregion
                    self._track_kb_usage(conversation_id, metadata['articles_used'], session)
                    # BUG FIX: Also update articles_referenced on conversation for quick access
                    if not conversation.articles_referenced:
                        conversation.articles_referenced = []
                    article_ids = [a.get('id') for a in metadata['articles_used'] if a.get('id')]
                    for article_id in article_ids:
                        if article_id not in conversation.articles_referenced:
                            conversation.articles_referenced.append(article_id)
                    # #region agent log
                    _log_debug(
                        "production_memory.py:140",
                        "After updating articles_referenced",
                        {
                            "conversation_id": conversation_id,
                            "articles_referenced_count": len(conversation.articles_referenced or []),
                            "articles_referenced": conversation.articles_referenced
                        },
                        "B"
                    )
                    # #endregion
