...
def process_message_async(self, customer_id: str, message: str, 
    ...
    try:
        ...
        # If the agent returned an internal error, surface it cleanly and
        # optionally escalate with a safe default priority.
        if 'error' in result:
        ...
        # If escalation needed, trigger notification using classification when available
        if result.get('escalated'):
            classification = result.get('classification')
            priority = 'high'
            if hasattr(classification, 'priority'):
                priority = getattr(classification, 'priority')
            elif isinstance(classification, dict) and 'priority' in classification:
