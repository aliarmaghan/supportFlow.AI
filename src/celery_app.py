...
celery_app = Celery(
    ...
    broker=os.getenv('REDIS_URL', 'redis://localhost:6379/0'),      # Uses Redis as the message broker.
    backend=os.getenv('REDIS_URL', 'redis://localhost:6379/0'),
    include=['src.core.async_tasks']  # Where our tasks are defined
)
...
celery_app.conf.task_routes = {
    'src.core.async_tasks.process_message_async': {'queue': 'messages'},
    'src.core.async_tasks.send_escalation_email': {'queue': 'notifications'},
    'src.core.async_tasks.generate_conversation_summary': {'queue': 'analytics'},
    'src.core.async_tasks.update_kb_index': {'queue': 'maintenance'},
}
